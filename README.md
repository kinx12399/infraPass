...
DB 간 통신이 단절되어 클러스터가 나뉘면 QNetd와의 연결, 멤버십, 설정된 알고리즘 등에 따라 중재 결과가 달라집니다. `ffsplit`과 `lms` 같은 알고리즘을 지원하지만 실제 적용 알고리즘과 tie-breaker 설정은 클러스터 설정으로 확인해야 합니다. QDevice가 표를 제공하는 상태와 단순히 QNetd에 연결된 상태도 구분해서 확인합니다. [QDevice 공식 매뉴얼](https://github.com/corosync/corosync-qdevice/blob/main/man/corosync-qdevice.8)
...
제출된 보안그룹 화면의 **현재 허용 규칙**을 Web / WAS / DB / 관리망의 4개 표로 정리했습니다. 각 표는 요구사항의 출발지 / 목적지 / 포트·프로토콜 / 방향 / 허용 사유 형식을 따릅니다. `In`은 해당 그룹이 적용된 VM의 수신, `Out`은 송신입니다. IPv6로 표시한 행을 제외하면 IPv4 규칙입니다. 표 아래에는 현재 규칙과 설계 의도를 대조한 내용을 적었습니다.
...
|---|---|---|---|---|
| `0.0.0.0/0` | Web 그룹 VM | TCP/80 | In | 웹 서비스 수신, 화면 설명 `web` |
| Bastion `192.168.20.7/32` | Web 그룹 VM | TCP/22 | In | 관리용 SSH |
| DB망 `192.168.23.0/24` | Web 그룹 VM | TCP/5403 | In | 화면상 QNetd 규칙, Web 적용 필요성 확인 |
...
**설계 의도와 대조:** 외부 사용자는 퍼블릭 LB의 HTTPS 443으로 접속하고, TLS 종료 후 Web의 HTTP 80으로 요청이 전달됩니다. 현재 Web:80은 모든 출발지를 허용하므로 퍼블릭 LB의 실제 백엔드·헬스 체크 출발지로 범위를 좁히는 것이 개선 항목입니다. 퍼블릭 LB의 공인 서비스 IP와 백엔드 출발지 IP가 같다고 가정하지 않습니다.

Web의 업무 송신 경로는 내부 LB `192.168.22.19:8080`입니다. QNetd는 관리망에 배치했으므로 Web 그룹의 TCP/5403 규칙은 실제 그룹 연결 대상을 확인한 후 필요성을 판단합니다.

#### 5.2 WAS — WAS 보안그룹

**서비스 대상:** WAS-01 / WAS-02 · **서브넷:** `192.168.22.0/24` · **역할:** 내부 LB의 API 요청 처리 및 DB 접근

제출된 WAS 화면에는 그룹 제목이 표시되지 않아 실제 보안그룹 이름은 별도 확인 대상입니다.
...
**설계 의도와 대조:** 서비스 요청은 Web → 내부 LB → WAS로 전달합니다. 화면의 TCP/8080 출발지는 Web망이 아닌 WAS망이지만, 내부 LB `192.168.22.19`가 이 대역에 있으므로 LB 경유 구조와 양립할 수 있습니다. 내부 LB의 SNAT·원본 IP 보존 방식과 헬스 체크 출발지를 확인하여 필요한 주소만 허용하고 규칙 설명을 정리합니다.
...
| `192.168.20.20/32` | DB 그룹 VM | TCP/5403 | In | 화면상 QNetd 규칙, 연결 방향 확인 |
...
**설계 의도와 대조:** MySQL의 서비스 수신은 WAS로 제한하고, 외부 및 Web에서의 직접 DB 접속은 허용하지 않습니다. TCP/3306의 허용 범위를 WAS 두 IP로 좁히면 같은 서브넷의 다른 VM 접근도 제한할 수 있습니다. DB 간 복제·클러스터 통신과 Bastion SSH는 별도 운영 목적의 예외입니다.

TCP/7789는 실제 DRBD 리소스 설정과 대조한 뒤 설명을 복제용으로 정리해야 합니다. Corosync 포트도 실제 transport 설정과 일치하는지 확인합니다. QDevice의 기본 연결 방향은 **DB → 관리망 QNetd:5403**이므로, 화면의 QNetd → DB 수신 규칙은 별도 서비스가 없다면 불필요할 수 있습니다. 필요한 QNetd 송신 연결은 현재 전체 송신 허용 범위에 포함됩니다.
...
**설계 의도와 대조:** 외부 관리자는 Bastion을 경유하여 내부 VM에 접근합니다. 관리 그룹이 Bastion과 QNetd에 함께 연결된다면 공인 IP의 SSH 허용이 어느 VM에 적용되는지 확인하고, 외부 SSH 진입은 Bastion으로 한정합니다. TCP/5403은 DB QDevice의 중재 요청을 받는 QNetd에 필요한 규칙이며, 펜싱용 포트가 아닙니다.
...
- **보안그룹 최소 권한화:** Web의 전체 출발지 허용과 QNetd 관련 규칙을 실제 LB·VM 배치에 맞게 정리하고, 송신 전체 허용을 필요한 목적지 중심으로 검토합니다.
