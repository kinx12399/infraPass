...
한 번에 VM 한 대만 중단하고, 복구와 정상 상태 확인을 마친 후 다음 VM을 시험합니다. 아래 항목은 **재시험에도 사용하는 검증 체크리스트**이며, 실제 수행한 시험과 관측 결과는 6.3에 별도로 기록합니다. 서비스 중단 시험과 VM 중단 시험은 구분하며, 개별 시험의 PASS가 아래 모든 세부 항목의 완료를 의미하지는 않습니다. Web·WAS는 LB가 생존 VM으로 요청을 전달하는지, DB는 활성 리소스가 대기 노드로 인계되는지 확인합니다.
