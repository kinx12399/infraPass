...
#### 6.2 VM 단일 장애 및 페일오버 체크리스트

한 번에 VM 한 대만 중단하고, 복구와 정상 상태 확인을 마친 후 다음 VM을 시험합니다. 아래 항목은 **시험 수행 시 체크하는 목록**이며, 6.1의 정상 상태 확인만으로 완료 처리하지 않습니다. Web·WAS는 LB가 생존 VM으로 요청을 전달하는지, DB는 활성 리소스가 대기 노드로 인계되는지 확인합니다.

**공통 준비**

- [ ] 시험 대상 VM, 중단 방법, 시험 시작 시각을 기록한다. 서비스 중지만 시험한 경우 VM 전체 장애와 구분한다.
- [ ] Web·WAS의 LB 멤버가 모두 정상이고, 외부 페이지·API 접속이 정상인지 확인한다.
- [ ] DB 두 노드 Online, DRBD 양쪽 UpToDate·Established, QDevice Connected, 총 3표·정족수 2인지 확인한다.
- [ ] 현재 활성 DB와 VIP 위치를 기록하고, 로그인 세션·기존 게시물·관심 목록을 준비한다.
- [ ] 중단한 VM을 다시 기동할 수 있는 복구 경로를 확보한다. DB 시험은 펜싱 미적용을 고려하여 기존 활성 노드의 실제 정지를 확인할 수 있는 조건에서 진행한다.

**Web-01 / Web-02 — 각각 한 대씩 중단 후 반복**

- [ ] 퍼블릭 LB가 중단한 Web을 비정상으로 판정하고 분산 대상에서 제외하는지 확인한다.
- [ ] 생존 Web의 Nginx와 `/healthz`가 정상인지 확인한다.
- [ ] 외부에서 새 요청을 반복하여 페이지와 정적 파일이 표시되고, 로그인·게시물 조회 API가 동작하는지 확인한다.
- [ ] 생존 Web의 접근 로그에서 요청 처리를 확인하고, 오류 발생 구간과 서비스 회복 시각을 기록한다.
- [ ] 중단한 Web을 복구한 뒤 LB 멤버가 정상으로 돌아오고 다시 요청을 처리하는지 확인한다.
- [ ] Web 두 대 모두 정상으로 복귀한 후 반대편 Web에서도 같은 시험을 수행한다.

**WAS-01 / WAS-02 — 각각 한 대씩 중단 후 반복**

- [ ] 내부 LB가 중단한 WAS를 비정상으로 판정하고 분산 대상에서 제외하는지 확인한다.
- [ ] 생존 WAS의 `infrapass` 서비스와 `/api/health/ready`가 정상인지 확인한다.
- [ ] 기존 로그인 상태에서 게시물·관심 목록을 조회하고, 새 게시물을 저장한 뒤 다시 조회한다.
- [ ] 생존 WAS의 로그에서 요청 처리를 확인하고, API 오류 구간과 회복 시각을 기록한다.
- [ ] 중단한 WAS를 복구한 뒤 readiness와 내부 LB 멤버 상태가 정상으로 돌아오는지 확인한다.
- [ ] WAS 두 대 모두 정상으로 복귀한 후 반대편 WAS에서도 같은 시험을 수행한다.

**활성 DB — 현재 Primary를 중단하여 대기 DB로 인계**

- [ ] `pcs status --full`과 `ip -brief address`로 시험 직전 활성 DB·VIP 위치를 확인한다. 노드 이름만으로 활성 역할을 판단하지 않는다.
- [ ] 기존 활성 DB의 실제 정지 또는 쓰기 중단을 확인한다. 네트워크 무응답만 확인된 경우 강제 승격하지 않고 노드 상태를 확인한다.
- [ ] 생존 DB에서 `corosync-quorumtool -s`로 Quorate Yes와 정족수 충족 여부를 확인하고, QDevice 연결·투표가 유지되는지 확인한다.
- [ ] `pcs status --full`에서 생존 DB의 `drbd_mysql`이 Promoted가 되고, `mysql_fs`·`mysql_service`·`mysql_vip`가 같은 노드에 Started인지 확인한다.
- [ ] `drbdadm status`에서 생존 DB가 Primary이고 로컬 디스크가 UpToDate인지 확인한다. 상대 VM 중단 중에는 복제 연결이 Established가 아닐 수 있으므로 복구 후 재확인한다.
- [ ] VIP `192.168.23.7`이 인계받은 노드에 존재하고, 두 노드에서 동시에 활성화되지 않는지 확인한다.
- [ ] WAS 두 대의 readiness가 회복되고, 외부에서 기존 데이터 조회·로그인 유지·신규 게시물 저장이 가능한지 확인한다.
- [ ] DB 중단 시각, 리소스 인계 시각, API 회복 시각, 실패 요청 및 데이터 보존 여부를 기록한다. 자동 인계와 운영자 개입 여부도 구분한다.
- [ ] 기존 DB를 복구한 뒤 클러스터 재참여, DRBD 재동기화 완료, 양쪽 UpToDate·Established, 총 3표 복원을 확인한다.
- [ ] 복구 후 Primary·MySQL·VIP가 한 노드에만 있고 다른 노드는 Secondary인지 확인한다. 원래 노드로의 자동 원복을 필수 성공 조건으로 삼지 않는다.

**대기 DB — 현재 Secondary 한 대 중단**

- [ ] 활성 DB의 MySQL·VIP가 유지되고 불필요한 역할 전환이 발생하지 않는지 확인한다.
- [ ] 활성 DB와 QDevice로 정족수가 유지되고, 외부 조회·쓰기 요청이 정상인지 확인한다.
- [ ] 대기 DB를 복구하여 Secondary 재참여, 재동기화 완료, 양쪽 UpToDate·Established 및 총 3표 복원을 확인한다.

**QNetd / Bastion — 각각 한 대씩 중단하는 영향 확인**

두 서버는 현재 단일 VM 구성이므로 대체 VM으로의 페일오버가 아닌, 서비스 영향과 복구를 확인합니다.

- [ ] QNetd 중단 시 DB 두 노드가 서로 연결된 상태에서 정족수와 MySQL·VIP가 유지되는지 확인한다. 이 시험 중 DB를 추가로 중단하지 않는다.
- [ ] QNetd 복구 후 서비스 active·TCP/5403 LISTEN, 두 DB의 QDevice Connected, 중재표를 포함한 총 3표 복원을 확인한다.
- [ ] Bastion 중단 시 외부 페이지·API 서비스가 유지되는지, Bastion을 통한 신규 관리 접속은 불가능해지는지 확인한다.
- [ ] Bastion 복구 후 허용된 관리자 경로에서 내부 VM으로 SSH 접속이 다시 가능한지 확인한다.

**시험 종료 및 증빙**

- [ ] 모든 VM, LB 멤버, DB 리소스, DRBD 복제 및 QDevice 투표가 시험 전 정상 상태로 돌아왔는지 확인한다.
- [ ] VM별 중단·감지·서비스 회복 시각, 사용자 영향, 데이터 보존 결과, 복구 결과를 기록하고 관련 로그·화면을 첨부한다.
- [ ] 실패하거나 수동 조치가 필요했던 항목은 원인과 조치를 기록하고, 재시험 전까지 완료 표시하지 않는다.
...
InfraPass는 Web·WAS·DB와 관리망을 분리하고, 계층별 접근 제어와 이중화 경로를 갖춘 3-Tier 인프라로 구축했습니다. 정상 상태 점검에서 DB 복제, 활성 리소스 배치, QDevice 정족수 중재를 확인했으며, 각 VM의 장애가 서비스에 미치는 영향은 위 체크리스트에 따라 검증하고 기록합니다.
